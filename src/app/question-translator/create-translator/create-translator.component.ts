...
export class CreateTranslatorComponent implements OnInit {
  ...
  questionCreation(form: NgForm): void {
    ...
    if (form.valid) {
      ...
      submitquest["translate"] = "yes"
      // }
      // submitquest["trans_submit"] = "yes"
      ...
      if (this.mode === 'EDIT') {
        this.questionService.updateQuestionAnswer(submitquest, this.current_editing_id).subscribe({
          next: (response) => {
            ...
            this.questionService.submitTranslatedQuestionAnswer(this.current_editing_id).subscribe({
              next: (response: any) => {
                alert("Question Transalated Successfully");
                this.questionService.Question.next({ Mode: 'LIST' });
              }
            })
          },
  ...
  checkrejected(){
    const approvedDate = new Date(this.editquestion.vetted);
    const rejectedDate = new Date(this.editquestion.rejected);

    if (approvedDate > rejectedDate) {
      return false;
    } else {
      return true;
    }
