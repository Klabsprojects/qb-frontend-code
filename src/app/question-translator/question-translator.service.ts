...
import { questionModel } from './question-translator.model';
...
export class QuestionCreationService {
  ...
  constructor(public http:HttpClient, private authService:authService) {
    console.log(this.GetQuestionDetail());
   }
  ...
  TranslateQues(data:any,id:number)
  {
    return this.http.put(`${'http://api-qgen.a2zweb.in/api/question/choice/'}${id}`, data, {headers:this.getHeaders()})
  }
  ...
  submitTranslatedQuestionAnswer(id:any){
    var data = {
      "trans_submit":"yes"
    }
    return this.http.put(this.questionCreationUrl+'/'+id,data,{headers:this.getHeaders()})
  ...
  GetQuestionDetail()
{
  const viewUrl = `${this.viewDetailsUrl}/${1}`;
  return this.http.get(viewUrl,{headers:this.getHeaders()});
