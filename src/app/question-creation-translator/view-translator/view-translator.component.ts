...
export class ViewTranslatorComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private questionAction: QuestionCreationService
  ) {}
  ngAfterViewInit() {
    this.loadMathJax();
  }
    ...
    this.questionService.Question.subscribe((index) => {
      this.questionService.getQuestionDetails(index.Index).subscribe({
        next: (response: any) => {
          ...
          this.choiceTextArray = this.question.choices.map(
            (choice: { choice_text: any }) => choice.choice_text
          );
          this.choiceTextArray_tn = this.question.choices.map(
            (choice: { choice_text_tn: any }) => choice.choice_text_tn
          );
          const choices = this.question.choices;
          if (this.question.format == 1) {
            this.answer = choices
              .filter(
                (choice: { choice_correct_yn: null }) =>
                  choice.choice_correct_yn !== null
              )
              .map(
            ...
            if (this.answer.length > 0) {
              ...
              if (correctChoiceIndex === 0 && choices.length > 0) {
                ...
                console.log(
                  'Invalid answer index or choices array is not long enough'
                );
              }
          ...
          next: (response: any) => {
            ...
            if (this.questions) {
              ...
              this.uniqueTypes = Array.from(
                new Set(
                  this.questions.map((question: { type: any }) => question.type)
                )
              );
            }
            ...
            if (this.question.format == 1) {
              this.answer = choices
                ...
                .map(
                  (choice: { choice_correct_yn: any }) =>
                    choice.choice_correct_yn
                );
              ...
              const allChoices = choices.map(
                (choice: any) => choice.choice_text
              );
              this.correctChoice = allChoices;

              const allChoices_tn = choices.map(
                (choice: any) => choice.choice_text_tn
              );
  ...
  choicenotejoin_tn(data: any): string {
    return data.choices
      .map(
        (choice: { choice_notes_tn: string | null }) => choice.choice_notes_tn
      )
